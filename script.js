document
  .getElementById("watering-form")
  .addEventListener("submit", function (event) {
    event.preventDefault();
    let currentWater = parseFloat(
      document.getElementById("current-water").value
    );
...
function calculateWatering(currentWater) {
  ...
  return {
    message: `YOO! You'll need: ${bestSolution.join(
      ", "
    )}. to get: ${closest}% water.`,
...
function calculateSoiling(currentSoil) {
  ...
  if (bestSolution.length === 0) {
    ...
    return `YOO! You'll need: ${bestSolution.join(
      ", "
    )}. to get: ${closest}% soil.`;
  }
}
